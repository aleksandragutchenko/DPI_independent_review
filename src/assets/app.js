...
function decisionCard(d,review=false){
  const reasoning=d.studentReasoning || (d.studentFinalAnswer && d.reasoningEmbedded ? 'Reasoning and certification are included in the student final answer.' : '');
  return `<details class="card decision ${review?'review-card':''} ${d.studentOverride?'override':''}" ${review?'open':''}><summary><span class="decision-id">${e(d.id)}</span><span class="decision-question">${e(d.question)}</span><span class="badge-row">${badges(d)}</span></summary><div class="decision-body"><div class="decision-grid">
    ...
    ${d.tier==='material_judgment'?field('Certification location',d.studentReasoning?'Separate certification field.':d.studentFinalAnswer?'Included within student final answer.':'Not recorded.'):''}
  </div></div></details>`;
}
function renderSummary(){
  ...
  return head('Independent review package','Financial reconstruction at 31 August 2026. Figures and conclusions are transferred from the attached workbook without silent adjustment.')+
    ...
    card('Workbook review status (preserved)',`<div class="table-wrap"><table><thead><tr><th>Item</th><th>Status</th><th>Meaning</th></tr></thead><tbody>${s.status.map(x=>`<tr><td>${e(x.item)}</td><td>${e(x.value)}</td><td>${e(x.meaning)}</td></tr>`).join('')}</tbody></table></div>`)+
...
function renderReview(){const mats=DATA.decisions.filter(d=>d.tier==='material_judgment'); return head('Assessor review','All 25 material judgments, student conclusions, challenges, effects, evidence, and unresolved conflicts in one compact review surface.','Assessor view')+`<div class="grid kpis"><div class="card kpi"><div class="kpi-label">Material judgments</div><div class="kpi-value">25</div></div><div class="card kpi"><div class="kpi-label">Student final answers</div><div class="kpi-value">${mats.filter(x=>x.studentFinalAnswer).length}</div></div><div class="card kpi"><div class="kpi-label">Student overrides</div><div class="kpi-value">${mats.filter(x=>x.studentOverride).length}</div></div><div class="card kpi"><div class="kpi-label">Agent disagreements</div><div class="kpi-value">${mats.filter(x=>x.disagreement).length}</div></div></div>`+card('Material review note',`<div class="notice info">A blank separate reasoning cell is not treated as incomplete when the student’s reasoning and certification are already written in the final-answer field.</div>`)+`<div class="decision-list section">${mats.map(d=>decisionCard(d,true)).join('')}</div>`+card('Unresolved uncertainties',renderGenericTable(DATA.tables.unresolved));}
