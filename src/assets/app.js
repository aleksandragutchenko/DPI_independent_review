...
function decisionCard(d,review=false){
  return `<details class="card decision ${review?'review-card':''} ${d.studentOverride?'override':''}" ${review?'open':''}><summary><span class="decision-id">${e(d.id)}</span><span class="decision-question">${e(d.question)}</span><span class="badge-row">${badges(d)}</span></summary><div class="decision-body"><div class="decision-grid">
    ...
    ${d.tier==='material_judgment'?`<div class="field full"><div class="field-label">Financial statement effect</div><div class="effect-grid">${['profit','cash',...(d.effects.ppe!==undefined?['ppe']:[]),'assets','liabilities','equity'].map(k=>`<div class="effect"><small>${({profit:'Profit',cash:'Cash',ppe:'PPE',assets:'Total assets',liabilities:'Liabilities',equity:'Equity'})[k]}</small>${eur(d.effects[k])}</div>`).join('')}</div>${d.effectsNote?`<p class="source-note">${e(d.effectsNote)}</p>`:''}</div>`:''}
